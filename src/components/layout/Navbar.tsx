    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6">
        ...
        <nav className="hidden lg:flex items-center gap-7">
          {links.map((l) => {
            // Check if the link item has a nested dropdown sub-list
            if ("dropdown" in l) {
              return (
                <div key={l.label} className="relative group/dropdown h-16 flex items-center">
                  <button className="flex items-center gap-1 font-display text-xs font-semibold uppercase tracking-[0.18em] text-white/80 transition-colors group-hover/dropdown:text-[#00B8FF]">
                    {l.label}
                    <ChevronDown className="h-3 w-3 transition-transform duration-200 group-hover/dropdown:rotate-180" />
                  </button>
                  
                  {/* Dropdown Menu Container Box - Unhides instantly on cursor hover */}
                  <div className="absolute top-full left-1/2 -translate-x-1/2 hidden group-hover/dropdown:block w-44 rounded-md border border-white/10 bg-[#0c0c0c] p-2 shadow-xl">
                    {l.dropdown.map((sub) => (
                      <Link
                        key={sub.to}
                        to={sub.to}
                        className="block rounded-sm px-4 py-2.5 font-display text-[11px] font-bold uppercase tracking-[0.15em] text-white/70 transition-colors hover:bg-white/5 hover:text-[#00B8FF]"
                      >
                        {sub.label}
                      </Link>
                    ))}
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/auth"
            className="hidden md:inline-flex items-center justify-center rounded-md border border-white/15 px-4 h-11 font-display text-xs font-bold uppercase tracking-[0.16em] text-white/85 hover:bg-white/5 hover:text-[#00B8FF]"
          >
            Team Login
          </Link>
          <Link
      ...
      {/* Mobile Menu Expansion Drawer Container */}
      <div
              ...
              return (
                <div key={l.label} className="border-b border-white/5">
                  <button
                    ...
                    className="flex w-full items-center justify-between py-3 font-display text-sm font-semibold uppercase tracking-[0.18em] text-white/85"
                  >
                    {l.label}
                    <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${mobileDropdownOpen ? "rotate-180 text-[#00B8FF]" : ""}`} />
                  </button>
                  
                  {/* Mobile Dropdown Sub-navigation links */}
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out pl-4 ${mobileDropdownOpen ? "max-h-24 opacity-100 pb-2" : "max-h-0 opacity-0"}`}>
                    {l.dropdown.map((sub) => (
                      <Link
                        key={sub.to}
                        to={sub.to}
                        onClick={() => { setOpen(false); setMobileDropdownOpen(false); }}
                        className="block py-2 font-display text-xs font-bold uppercase tracking-[0.15em] text-white/60 hover:text-[#00B8FF]"
                      >
                        {sub.label}
                      </Link>
                    ))}
